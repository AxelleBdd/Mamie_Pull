<template>
  <div
    :class="[
      'flex h-full flex-col rounded-xl shadow-sm border-dark-purple border overflow-hidden transition hover:-translate-y-1 hover:shadow-lg',
  ...
  >
    <!-- Image -->
    <div
      class="relative h-48 w-full bg-grey-purple flex items-center justify-center overflow-hidden"
    >
      <img
        v-if="product.image"
        :src="product.image"
      ...
      />
      <span v-else aria-hidden="true" class="text-6xl text-grey-purple">
        📦
      </span>
    </div>
