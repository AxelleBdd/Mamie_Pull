<template>
  <div class="max-w-400 mx-auto px-4 md:px-8 lg:px-12 py-4 bg-white-purple-100 min-h-screen">
    <!-- Header -->
    ...
    <nav aria-label="Fil d'Ariane" class="mb-6 flex items-center gap-2 sm:text-lg md:text-xl text-dark-purple-700">
      ...
      <router-link to="/products" class="hover:underline">Tous nos produits</router-link>
      <span aria-hidden="true"> > </span>
      <router-link 
      ...
      <span class="font-medium" aria-current="page">{{ product?.title || 'Chargement...' }}</span>
    </nav>

    <!-- Back button -->
    <button
      @click="goBack"
      class="mb-6 inline-flex items-center gap-2 px-4 py-2 rounded-lg text-lg bg-dark-purple-700 
             text-white-purple-100 hover:bg-highlight-purple-500 transition"
    ...
    <div v-else-if="product" class="flex flex-col lg:flex-row gap-8">
      ...
      <div class="lg:w-1/2">
        <div class="sticky top-4">
          ...
          <div class="relative rounded-xl overflow-hidden bg-grey-purple-400 aspect-square flex items-center justify-center">
            <img
              v-if="product.image"
              :src="product.image"
              :alt="product.title"
            ...
            />
            <span v-else class="text-9xl text-grey-purple-300" aria-hidden="true">
              📦
            </span>
            
            <!-- Heart Icon -->
            <button
              class="absolute top-4 right-4 w-10 h-10 rounded-full bg-white-purple-100 
      ...
      <div class="lg:w-1/2 flex flex-col">
        <!-- Description Section -->
        <div class="mb-8">
          <h2 class="text-2xl font-semibold font-heading text-dark-purple-700 mb-4">
            Description
          </h2>
          <p class="text-base sm:text-lg text-dark-purple-700 leading-relaxed whitespace-pre-line">
            {{ product.description }}
          </p>
        </div>
        ...
        <!-- Contact Button -->
        <button
          class="mt-auto w-full py-4 px-6 bg-dark-purple-700 text-white-purple-100 
                 rounded-lg text-lg sm:text-xl font-medium font-body 
                 hover:bg-highlight-purple-500 transition active:scale-95"
    ...
    <div v-else class="text-center py-16" role="alert">
      ...
      <router-link
        ...
        class="inline-block px-8 py-3 rounded-lg font-medium text-white bg-dark-purple-700
               hover:bg-highlight-purple-500 transition"
...
const loadProduct = async () => {
  ...
  try {
    ...
    error.value = 'Impossible de charger le produit. Vérifiez que le backend est bien lancé.';
